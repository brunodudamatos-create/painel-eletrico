// ================================================================
// api/dados.js — Versão Unificada (ES Modules, Alarmes Completos, Termostato e Telegram)
// ================================================================
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2. Coleta do Termostato (Temperatura Atual e Setpoint dinâmico)
      ...
      try {
        ...
        if (dataTermo.result && Array.isArray(dataTermo.result)) {
          ...
          if (itemSet && itemSet.value != null) {
            let valSet = itemSet.value;
            temperatura.temp_set = (valSet > 100 ? valSet / 10 : Number(valSet)).toFixed(1);
          }
        } else {
    ...
    // 3. Lógica de Alarmes (Tensão e Corrente para Fases A, B e C)
    let alertas = [];
    ...
    // Limites de Corrente (Ajustável conforme o limite do seu disjuntor/equipamento, ex: 50A)
    ...
    // 4. Controle de Estado (Supabase - Alertas e Notificações Telegram)
    ...
    if (alertas.length > 0) {
      ...
      if (estadoAnterior?.em_alerta) {
        deveEnviarTelegram = true;
        textoMensagem = `✅ *SISTEMA NORMALIZADO*\n_Painel: Brasileira Distribuidora_\n\nAs grandezas elétricas e operacionais retornaram aos níveis normais.`;
    ...
    // 5. Persistência no Histórico (Incluindo Temperatura e Setpoint atualizados no Supabase)
    const dadoHistorico = {
      ...eletrico,
      temp_atual: temperatura.temp_atual,
      temp_set: temperatura.temp_set
    };

    const { error: dbError } = await supabase
      .from('telemetria_eletrica')
      .insert([dadoHistorico]);
    ...
    return res.status(200).json({
      ...
      eletrico,
      temperatura,
      banco_dados: dbError ? "Erro gravação" : "Gravação Sucesso"
    });
